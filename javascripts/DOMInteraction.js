...
domInter.attSearch = function (area, attractions) {
...
//compares user input to attraction name, finds the area id and highlights the appropriate area
domInter.typeSearch = function (type, attractions) {
  let userInput = textInput.value.toLowerCase();
  // domInter.clear(area);
  $(".side-bar").empty();
  $.each(attractions, function (name, value) {
    let lowerName = value.typeName.toLowerCase();
    if (lowerName.indexOf(userInput) !== -1) {
      $("#" + value.area_id).toggleClass("highlight");
     domInter.atts(value);
    }
...
domInter.areaSearch = function (area, attractions) {
  domInter.clear(area);
